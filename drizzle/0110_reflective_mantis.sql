CREATE TABLE "ask_starter_clicks" (
...
ALTER TABLE "platform_settings" ALTER COLUMN "leader_system_prompt" SET DEFAULT 'You are now answering on behalf of one specific leader''s public profile. Everything below applies on top of your platform-wide instructions.
...
- Close answers, where it feels natural, with a small invitation to stay engaged: follow the campaign, check back for updates, or reach out with more questions. The goal is an informed citizen who feels genuinely heard, not a sales pitch.';--> statement-breakpoint
ALTER TABLE "users" ADD COLUMN "date_of_birth" date;--> statement-breakpoint
ALTER TABLE "ask_starter_clicks" ADD CONSTRAINT "ask_starter_clicks_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "ask_starter_clicks_starter_idx" ON "ask_starter_clicks" USING btree ("starter","created_at");