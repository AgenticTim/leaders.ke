<script lang="ts">
	...
	const meta = $derived(
		sentiment === 'positive'
			? { label: 'Positive coverage', tone: 'text-tone-positive', path: 'M6 2.5 10.2 9.5H1.8Z' }
			: sentiment === 'negative'
				? { label: 'Negative coverage', tone: 'text-tone-negative', path: 'M6 9.5 1.8 2.5h8.4Z' }
				: { label: 'Neutral coverage', tone: 'text-tone-neutral', path: 'M1.8 5h8.4v2H1.8Z' }
	);
</script>
...
{#if sentiment}
	...
	>
		<svg viewBox="0 0 12 12" class="size-4" fill="currentColor" aria-hidden="true">
			<path d={meta.path} />
		</svg>
	</span>
{/if}
