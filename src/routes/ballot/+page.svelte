<script lang="ts">
	...
	// (e.g. straight back to /ballot after casting a vote) isn't a deliberate "clear
	// the selection", so it must never stomp whatever GeoSelect just restored from the
										...
										<span class="min-w-0">
											<span class="flex items-center gap-1 font-semibold text-heading justify-center">
												...
												{#if candidate.verified}
													<VerifiedIcon title="An admin has manually confirmed this candidacy's IEBC certificate." />
												{/if}
											</span>
